...
export const isDateInSanFuPeriod = (now: moment.Moment) => {
    const period = getSanFuPeriod(now.year());
    if (!period) return false;
    return now.isSameOrAfter(period.start, 'day') && now.isSameOrBefore(period.end, 'day');
};
